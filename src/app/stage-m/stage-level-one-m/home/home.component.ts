...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //first term
       this.store_service.stage_levone_firTerm().subscribe(data =>{
    ...
    //second term
    this.store_service.stage_levone_secTerm().subscribe(data =>{
    ...
    /******************* start typeit ******/
      new TypeIt("#big_stage_one", {
        strings: "الفرقة الاولي ",
        speed:100,
        cursor: false,
      }).go();
      new TypeIt("#small_stage_one", {
        strings: " مسرح ",
        speed:100,
        cursor: false,
        startDelay: 1800
